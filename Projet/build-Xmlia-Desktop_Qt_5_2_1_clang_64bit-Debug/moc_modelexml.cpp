/****************************************************************************
** Meta object code from reading C++ file 'modelexml.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.2.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../Xmlia/modelexml.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'modelexml.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.2.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_ModeleXml_t {
    QByteArrayData data[10];
    char stringdata[100];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_ModeleXml_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_ModeleXml_t qt_meta_stringdata_ModeleXml = {
    {
QT_MOC_LITERAL(0, 0, 9),
QT_MOC_LITERAL(1, 10, 16),
QT_MOC_LITERAL(2, 27, 0),
QT_MOC_LITERAL(3, 28, 8),
QT_MOC_LITERAL(4, 37, 1),
QT_MOC_LITERAL(5, 39, 7),
QT_MOC_LITERAL(6, 47, 12),
QT_MOC_LITERAL(7, 60, 12),
QT_MOC_LITERAL(8, 73, 6),
QT_MOC_LITERAL(9, 80, 18)
    },
    "ModeleXml\0onNodeNameUpdate\0\0QDomNode\0"
    "n\0newName\0onNodeDelete\0onNodeInsert\0"
    "parent\0onAboutToBeRemoved\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_ModeleXml[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       4,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    2,   34,    2, 0x06,
       6,    1,   39,    2, 0x06,
       7,    2,   42,    2, 0x06,
       9,    1,   47,    2, 0x06,

 // signals: parameters
    QMetaType::Void, 0x80000000 | 3, QMetaType::QString,    4,    5,
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void, 0x80000000 | 3, 0x80000000 | 3,    8,    4,
    QMetaType::Void, 0x80000000 | 3,    2,

       0        // eod
};

void ModeleXml::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        ModeleXml *_t = static_cast<ModeleXml *>(_o);
        switch (_id) {
        case 0: _t->onNodeNameUpdate((*reinterpret_cast< QDomNode(*)>(_a[1])),(*reinterpret_cast< QString(*)>(_a[2]))); break;
        case 1: _t->onNodeDelete((*reinterpret_cast< QDomNode(*)>(_a[1]))); break;
        case 2: _t->onNodeInsert((*reinterpret_cast< QDomNode(*)>(_a[1])),(*reinterpret_cast< QDomNode(*)>(_a[2]))); break;
        case 3: _t->onAboutToBeRemoved((*reinterpret_cast< QDomNode(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (ModeleXml::*_t)(QDomNode , QString );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ModeleXml::onNodeNameUpdate)) {
                *result = 0;
            }
        }
        {
            typedef void (ModeleXml::*_t)(QDomNode );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ModeleXml::onNodeDelete)) {
                *result = 1;
            }
        }
        {
            typedef void (ModeleXml::*_t)(QDomNode , QDomNode );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ModeleXml::onNodeInsert)) {
                *result = 2;
            }
        }
        {
            typedef void (ModeleXml::*_t)(QDomNode );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ModeleXml::onAboutToBeRemoved)) {
                *result = 3;
            }
        }
    }
}

const QMetaObject ModeleXml::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_ModeleXml.data,
      qt_meta_data_ModeleXml,  qt_static_metacall, 0, 0}
};


const QMetaObject *ModeleXml::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *ModeleXml::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_ModeleXml.stringdata))
        return static_cast<void*>(const_cast< ModeleXml*>(this));
    return QObject::qt_metacast(_clname);
}

int ModeleXml::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void ModeleXml::onNodeNameUpdate(QDomNode _t1, QString _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void ModeleXml::onNodeDelete(QDomNode _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void ModeleXml::onNodeInsert(QDomNode _t1, QDomNode _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}

// SIGNAL 3
void ModeleXml::onAboutToBeRemoved(QDomNode _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}
QT_END_MOC_NAMESPACE
